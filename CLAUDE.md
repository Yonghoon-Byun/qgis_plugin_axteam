...
| **civil_planner** | 1.0.6 → 2.0.0 작업중 | 토목 관로 설계 7단계 위자드 (UNION ALL 최적화, v2 개편 진행중) |

---

## civil_planner 상세 (현재 v1.0.6)
...
## [현재 작업] civil_planner v2 개편 계획

> 상세 계획: `docs/civil_planner_v2_작업정리.md` 및 `.omc/plans/civil-planner-v2.md` 참고

### v2 위자드 구조 (변경 후)

| Step | 이름 | 파일 | 변경 |
|------|------|------|------|
| 1 | 작업환경 설정 | `step1_setup.py` | 유지 |
| 2 | 범위 설정 | `step2_boundary.py` | 유지 |
| 3 | **생성방식 설정** | `step3_save_mode.py` | **신규** — 영구/임시 선택 + 폴더 지정 |
| 4 | **서버 데이터 로드** | `step4_server_data.py` | 리네임 + 경과시간 + 저장경로 |
| 5 | **로컬 데이터 로드** | `step5_local_data.py` | 재작성 — 드래그앤드롭 + 인코딩 처리 |
| 6 | 정리 및 스타일 | `step6_organize.py` | 순서 이동 |
| 7 | **계획시설 레이어 생성** | `step7_facility.py` | 재작성 — 9색×2타입 템플릿 스타일 |

### 요구사항 13건 요약

**레이어 변경:**
- R3: 행정경계 4분리 (시도/시군구/읍면동/법정리, 해당 레벨 명칭만 라벨)
- R4: 하천경계 라벨 추가 (하천명/소하천명)
- R5~R7: 도로경계선, 연속지적도, 단지유치업종 제거
- R8: 건물 → "건물 합정보_마스터"로 변경 (**DEFERRED** — DB 함수명 미정)

**신규 기능:**
- R1: 레이어 생성방식 선택 (영구+폴더 / 임시)
- R2: 데이터 로딩 경과시간 표시
- R11: 로컬 데이터 인코딩 처리 (`setProviderEncoding`, EUC-KR→CP949→UTF-8 폴백)

**위자드 재구성:**
- R9: "데이터 로드" → "서버 데이터 로드" + 저장 경로
- R10: "지장물 연동" → "로컬 데이터 로드" (드래그앤드롭)
- R12: "정리 및 스타일" → 6번으로 이동
- R13: "관로설계" → "계획시설 레이어 생성" (이중점선/이중실선 × 9색 템플릿)

### 실행 순서

```
Phase 0: 파일 리네이밍/스캐폴드 (git mv + 신규 파일 생성)
Phase 1: 레이어 제거 (R5/R6/R7) + 스타일 XML (R13)         [병렬]
Phase 2: 행정경계 분리 (R3) + 하천 라벨 (R4)                [병렬]
Phase 3: 생성방식 (R1) + 로컬로드 (R10/R11) + 계획시설 (R13) [병렬]
Phase 4: 서버로드 (R2/R9) + 정리/스타일 (R12)               [병렬]
Phase 5: wizard_dialog 통합 → 구 파일 삭제                   [순차]
BLOCKED: R8 건물 레이어 교체 (DB 함수명 대기)
```
...
| 항목 | 상태 | 필요한 조치 |
|------|------|-----------|
| R8 "건물 합정보_마스터" DB 함수명 | DEFERRED | DB 함수/테이블명 확인 후 `USE_BUILDING_MASTER` 플래그 활성화 |
| R4 `river_boundary_clip` 반환 필드 | 미확인 | DB 스키마 조회 (없으면 자동 스킵) |
| R3 법정리(LENGTH=10) 데이터 충분성 | 미확인 | 실 데이터 테스트 필요 |
